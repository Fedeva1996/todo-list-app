...
const TodoDoneList = (props) => {
  ...
  //console.log(lista);
  const listItemFiltered = lista.filter((item) => item.done).map((item) => {
    return (
      <div
        key={item._id}
        className="my-2 flex items-center justify-between rounded-md bg-slate-700 p-2"
      >
  ...
  return (
    <div>
      {listItemFiltered.length
        ? listItemFiltered
        : "Lista vacia, aún no completaste ninguna tarea :("}
    </div>
  );
};
